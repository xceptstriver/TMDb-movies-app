...
const CastScreen = (props) => {
  const {bkgStyle, isDarkMode, navigation, castId, profilePic} = props;
  ...
  const fetchDetails = async (castId) => {
    let url = `https://api.themoviedb.org/3/person/${castId}?api_key=${TMDB_API.API_KEY}&language=en-US`;
    try {
      await fetch(url)
  ...
  const fetchExternalId = async (castId) => {
    let url = `https://api.themoviedb.org/3/person/${castId}/external_ids?api_key=${TMDB_API.API_KEY}&language=en-US`;
    try {
      await fetch(url)
        .then((response) => response.json())
        .then((data) => {

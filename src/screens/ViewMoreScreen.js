...
const ViewMoreScreen = (props) => {
  ...
  const fetchMovies = async (setAllMovies, category, pgNum) => {
    ...
    switch (category) {
      case 'Upcoming':
        url = `https://api.themoviedb.org/3/movie/upcoming?api_key=${TMDB_API.API_KEY}&language=en-US&page=${pgNum}&region=us`;
        break;
      case 'Popular':
        url = `https://api.themoviedb.org/3/movie/popular?api_key=${TMDB_API.API_KEY}&language=en-US&page=${pgNum}`;
        break;
      case 'Top Rated':
        url = `https://api.themoviedb.org/3/movie/top_rated?api_key=${TMDB_API.API_KEY}&language=en-US&page=${pgNum}`;
        break;
      default:
        url = `https://api.themoviedb.org/3/movie/popular?api_key=${TMDB_API.API_KEY}&language=en-US&page=${pgNum}`;
    }
